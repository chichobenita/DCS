...
// From PC to MCU
typedef enum RadarCommandToMCU
{
    ...
    RADAR_CMD_START_SCRIPT             = 0x13


}RadarCommandToMCU;


enum RadarLDRCommand{
    RADAR_MSG_NEW_CALIBRATION_SAMPLE    =0x01,
    RADAR_MSG_NEW_SAMPLE                =0x02

};

// From MCU to PC
enum RadarCommandFromMCU {
...
typedef struct {
    uint8_t code;
    char msg[12]; // ASCII string (null-terminated optional)
} payload_error_t;

// RADAR_MSG_LDR_CALIB_RESULT
typedef struct {
    uint8_t x;
    uint8_t degree;
    uint8_t success;
} payload_ldr_calib_result_t;
