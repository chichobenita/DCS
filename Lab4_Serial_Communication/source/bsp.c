...
*/
  ...
  // PushButtons Setup
  PBsArrPortSel &= ~(PB1);
  PBsArrPortDir &= ~(PB1);  // PB1 connected to P1.0
  ...
  PBsArrIntEn |= 0x07;               // only P1.3 can interrupt
...
void UARTconfig(void){
    ...
    P1SEL2 |= UART_MODE;         // בחירה ב-Function 2 — החיבור ל-USCI_A0
    ...
    // הגדרת UART
     ...
     UCA0MCTL = UCBRS0;             // Modulation - מאפשר כיוונון למחלק הראשי כדי שה-Baud Rate הסופי יהיה מדויק ככל האפשר.
     UCA0CTL1 &= UART_RESET;        // מאפשר תקשורת חומרתית
     IE2 |= UART_ENABLE;            // אפשר פסיקת קלט UART
}
