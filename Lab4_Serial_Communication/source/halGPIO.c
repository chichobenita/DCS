...
volatile int X = 500;
volatile unsigned int x_ready = 0;
    ...
    // write the code of the timer mm:ss
       //LPM0_EXIT;
           ...
           if (state==state2)
           {
               ...
               const float SMCLK_Hz = 1048576.0f;
               unsigned int period = (unsigned int)(SMCLK_Hz / tone_series_hz[tone_idx]);
...
__interrupt void USCI0RX_ISR(void)
{
    ...
    else if ((c == '#') && msg_in_progress) {
        msg_buf[msg_idx] = '\0';
        msg_in_progress = 0;

        // handle the msg
        api_handle_msg((char*)msg_buf);
    }
    else if (msg_in_progress && msg_idx < MSG_BUF_SIZE - 1) {
        msg_buf[msg_idx++] = c;
    }
